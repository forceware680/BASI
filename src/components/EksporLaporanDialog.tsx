// components/EksporLaporanDialog.tsx — Dialog Ekspor & Rekapitulasi Laporan Berdasarkan Periode.
...
export function EksporLaporanDialog({
  ...
  const handleExportCsv = () => {
    ...
    const rowsCsv = data.map((r, i) => [
      ...
      escapeCsv(r.status === "SELESAI" ? "SELESAI" : "MENUNGGU BUKTI"),
      escapeCsv(r.file_path || r.status === "SELESAI" ? "Ada" : "Belum Ada"),
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
        ...
        <div className="space-y-4 py-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <span className="text-[11px] text-slate-500 mb-1 block">Dari Tanggal:</span>
                <input
                  type="date"
                  value={dateFrom}
                  onChange={(e) => setDateFrom(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-medium text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
              <div>
                <span className="text-[11px] text-slate-500 mb-1 block">Sampai Tanggal:</span>
                <input
                  type="date"
                  value={dateTo}
                  max={todayIso()}
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-3.5 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Jumlah data yang sesuai:</span>
              <span className="font-bold text-indigo-950 text-sm">{filteredData.length} Berkas</span>
            </div>
        ...
        {/* Tombol Aksi */}
        <div className="flex flex-wrap items-center justify-end gap-2.5 border-t border-slate-200 pt-4">
          <button
            type="button"
            onClick={onClose}
