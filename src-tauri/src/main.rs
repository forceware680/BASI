...
async fn open_bukti_path(app: tauri::AppHandle, path: String) -> Result<(), String> {
    ...
    // Ambil nama file dan koreksi_id jika memungkinkan dari path
    let parts: Vec<&str> = path_norm.split('/').filter(|s| !s.is_empty()).collect();
    let (k_id, fname) = if parts.len() >= 2 {
        (parts[parts.len() - 2], parts[parts.len() - 1])
    } else {
        ("", std::path::Path::new(&path_norm).file_name().map(|n| n.to_str().unwrap_or("")).unwrap_or(""))
    };

    if cfg.mode == "online" && !cfg.storage_api_url.trim().is_empty() && !k_id.is_empty() && !fname.is_empty() {
